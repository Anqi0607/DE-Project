...
def validate_raw_data_with_spark(bucket_name: str, gcs_prefix: str, state: str, **kwargs):
    ...
    Validations include:
      ...
      4. Calculating the null value ratio for each column.
    ...
    # Create SparkSession using get_spark_session
    ...
    for blob in monthly_blobs:
        ...
        # 4. Calculate and print the null value ratio for each column
        for col in df.columns:
            # We already know "M" refers to a missing value.
            null_count = df.filter((df[col].isNull()) | (df[col] == "M")).count()
