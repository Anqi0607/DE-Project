{{ config(
    ...
    unique_key='generated_primary_key'
)
...
with source as (
    ...
    select * from {{ source('staging', 'bronze_table') }}
...
renamed as (
    ...
    select
        {{ dbt_utils.generate_surrogate_key(['valid', 'station']) }} as observation_id,
...
where valid > (
    ...
    select coalesce(max(valid), '1900-01-01')
    from {{ this }}
)
